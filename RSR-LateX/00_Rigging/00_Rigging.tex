...
\end{equation}
$\vec{p}$ (\enquote{gut}) und $\vec{q}$ (\enquote{schlecht}) sind Vektoren welche die Einbettungen repäsentieren. Der Vektor $\vec{\Gamma}$ enthält das Gewicht, $n$ und $m$ stehen für die Anzahl der \enquote{guten} bzw. \enquote{schlechten} Einbettungen. Idealerweise sollte $\vec{\Gamma}$ so gewählt werden, das $c$ gegen $1$ geht. 
