...
In Abbildung \ref{embedding_fig1} ist das Skelett einbinden vereinfacht graphisch dargestellt.
\begin{figure}[t]
	\centering
...
\begin{figure}[t]
	...
	\caption[Schrittweise Darstellung wie ein Graph im Modell gefunden wird]{ a) Mediale Fläschen b) Dichteste Kugelpackung c) Erzeugter Graph d) Orginal und vereinfachtes Skelett. Entnommen aus \cite{embedding}}
	\label{embedding_fig2}
\end{figure}

\subsubsection{Volumenreduktion I}
\label{sec:medial_surface}
Der erste Schritt zur Volumenreduzierung ist das finden der medialen Fläschen. Diese sind ein equivalent der Mittelachse von zweidimensionalen Fläschen im dreidimensionalen Raum. Ein Punkt liegt auf dieser Fläsche, wenn er mehr als eine minimale Distanz von sich zur Körperoberfläsche besitzt.
Man erwartet das dass eingebettede Skelett auf dieser Fläsche zu finden ist. In Abbildung \ref{embedding_fig2} a) sind diese medialen Fläschen eines Meshs zu sehen.

\subsubsection{Volumenreduktion II}
\label{sec:sphere_packing}
Um das Volumen weiter zu reduzieren wird ein Graph erstellt, welcher die Gelenke als Knoten und die Knochen als Kanten hat. Um diesen Graph zu erzeugen wird eine Methode ähnlich der Methode zur Findung Dichtesten Kugelpackung\footnote{\label{foot:1}Bei dieser Methode wird versucht Kugeln mit dem Gleichen Radius in einem dreidimensionalen Körper zu plazieren, wobei die Kugeln sich nur berühren aber nicht überlappen dürfen. Ziel dieses Verfahrns ist es, einen möglichst minimalen Freiraum zwischen den Kugeln zu finden.} verwendet. Jedoch dürfen sich bei dieser Methode die Kugeln überlappen.

Jeder Punkt auf der medialen Fläsche dient als Mittelpunkt einer Kugel und hat als Radius seine Distanz zur Oberfläsche des Meshs. Als Startpunkt wird der Punkt genommen, wessen Distanz zur Oberfläsche am größten ist. Es dürfen nur Kugeln hinzugefügt werden, welche nicht den Mittelpunkt einer anderen Kugel umschließen. 
...
Um das vorgegebene Skelett nun in das Modell einzubinden, wird der erzeugte Graph $G = (V, E)$ benötigt. Der erste Schritt hierbei ist es das vorgegebene Skelett zu vereinfachen. Das orginal Skelett hat $s$ Gelenke das reduzierte Skelett $r$ Gelenke. 
...
Bei der Allgemeingültigkeit ist der Algorithmus stark eingeschränkt auf das vorgegebene Skelett, in diesem Fall auf humanuide Modelle. Ein weiterer Nachteil ist ebenfalls das kleine Modelle mit vielen kleinen Knochen (z.B. Schlangen) zu Problemen führen. Da diese durch die Straffunktionen ausgeschlossen werden. 
